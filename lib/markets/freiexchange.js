...
//
//  Get Market From Freiexchange
//
function get_summary(coin, exchange, cb) {
  var summary = {};
  var req_url = base_url + '/public/' + coin.toUpperCase();
...
function get_trades(coin, exchange, cb) {
  var req_url = base_url + '/public/trades/' + coin.toUpperCase() + '/' + exchange.toUpperCase();
  request({uri: req_url, json: true}, function (error, response, body) {
    if (error) {
      return cb(error, null);
    } else if (body.error !== true) {
...
function get_orders(coin, exchange, cb) {
  var req_url = base_url + '/public/orderbook/' + coin + '/' + exchange;
  request({uri: req_url, json: true}, function (error, response, body) {
    if (error) {
      return cb(error, null);
    } else if (body.error !== true) {
